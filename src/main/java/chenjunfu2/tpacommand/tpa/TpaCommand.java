...
{
	static class PlayerTpaSuggestion implements SuggestionProvider<ServerCommandSource>
	{
		@Override
	...
	private final static SuggestionProvider<ServerCommandSource> SUGGEST_TPA_PLAYERS = new PlayerTpaSuggestion();
	
	public static void RegisterCommand(CommandDispatcher<ServerCommandSource> dispatcher)
	{
		
		LiteralArgumentBuilder<ServerCommandSource> tpaCommand =
			CommandManager.literal("tpa")
			              .requires(ServerCommandSource::isExecutedByPlayer)
			              .then(CommandManager.argument("player", EntityArgumentType.player())
			                                  .suggests(SUGGEST_TPA_PLAYERS)
			                                  .executes(context -> tpaPlayer(context, EntityArgumentType.getPlayer(context, "player"))));
		
		LiteralArgumentBuilder<ServerCommandSource> tpahereCommand =
		...
		LiteralArgumentBuilder<ServerCommandSource> calcelCommand =
		...
		LiteralArgumentBuilder<ServerCommandSource> acceptCommand =
		...
		LiteralArgumentBuilder<ServerCommandSource> denyCommand =
	...
	private static int tpaPlayer(CommandContext<ServerCommandSource> context, ServerPlayerEntity target)
	{
		...
		Text msg = TpaRequest.AddRequest(new PlayerData(source), new PlayerData(target), TpaDirection.TPA_MODE);
		if (msg != null)
		{
	...
	private static int tpaHere(CommandContext<ServerCommandSource> context, ServerPlayerEntity target)
	{
		...
		Text msg = TpaRequest.AddRequest(new PlayerData(source), new PlayerData(target), TpaDirection.TPAHERE_MODE);
		if (msg != null)
		{
			context.getSource().sendError(msg);
			return 0;
	...
	private static int tpaAccept(CommandContext<ServerCommandSource> context, ServerPlayerEntity target)
	{
		...
		if (dir == TpaDirection.TPA_MODE)//source是accept的执行者，tpa模式的传送目标
		{
			from = target;
			to = source;
		}
		else if (dir == TpaDirection.TPAHERE_MODE)//source是accept的执行者，tpahere模式的传送者
		{
			from = source;
			to = target;
		}
